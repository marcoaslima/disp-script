...
namespace Disp.Internal
{
    public class Loader
    {
        ...
        public System.String Execute(ConsoleCommand command, Int32 Line = 0)
        {
            // Validate the command name:
            if (!_commandLibraries.ContainsKey(command.LibraryClassName))
            {
                return string.Format(Definitions.badCommandMessage, command.LibraryClassName, Line);
            }

            var methodDictionary = _commandLibraries[command.LibraryClassName];
            if (!methodDictionary.ContainsKey(command.Name))
            {
                return string.Format(Definitions.badCommandMessage, command.LibraryClassName + "." + command.Name, Line);
            }
            ...
            Type commandLibaryClass =  current.GetType("Disp." + command.LibraryClassName);
            ...
            try
            {
                ...
                types[0] = this.Ambiente.GetType();

                ConstructorInfo ctor = typeInfo.GetConstructor(types);
                var argValues = new object[] { this.Ambiente };
                object instance = ctor.Invoke(argValues);

                var result = typeInfo.InvokeMember(
                    command.Name,
                    BindingFlags.InvokeMethod | BindingFlags.Public | BindingFlags.Instance,
                    null, instance, inputArgs);
